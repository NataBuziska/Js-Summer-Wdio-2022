 ...
 */
 ...
 await browser.pause(3000);
 await $("//input[@placeholder='Going to']").click();
 await browser.pause(3000);
 ...
 await browser.pause(3000);
     ...
     const allOctDateElements = await $$('//h2[text()="October 2022"]/following-sibling::table//button[not(@disabled)]');
     for (const dateElement of allOctDateElements) {
        const date = await dateElement.getAttribute('data-day');
